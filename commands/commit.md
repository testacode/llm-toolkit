...
1. Analyze the current git changes using `git status` and `git diff --staged` or use the `gh` cli command (always use a --limit 10 to avoid excess of tokens)
...
9. Never mention Claude Code in the commit message
10. Never push the commit
